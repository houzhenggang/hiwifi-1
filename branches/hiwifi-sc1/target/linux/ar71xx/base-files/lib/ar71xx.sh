...
ar71xx_uboot_version() {
	dd if=/dev/mtdblock0 ibs=1 skip=65472 2>/dev/null | dd of=/tmp/sysinfo/boot_version bs=20 skip=0 count=1 2>/dev/null
...
get_boot_version() {
	get_image "$@" | dd ibs=1 skip=65472 2>/dev/null | dd bs=20 skip=0 count=1 2>/dev/null | awk '{print $2}'
}
...
tw_get_mac() {
	...
	case "$board" in
		tw150* | HC6341* | HC6661* | HC6342* | HB6881*)
